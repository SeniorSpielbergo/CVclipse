...
/**
 * The CvTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * CvTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class CvTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.mopp.CvTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.mopp.CvTaskItem> foundItems = new java.util.ArrayList<de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.mopp.CvTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.mopp.CvTaskItem(keyword, message, localLine, offset, end));
