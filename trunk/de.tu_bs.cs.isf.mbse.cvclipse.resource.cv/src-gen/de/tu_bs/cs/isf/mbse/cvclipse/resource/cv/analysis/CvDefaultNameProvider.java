...
public class CvDefaultNameProvider implements de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ICvNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.util.CvEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
