 ...
 */
public class CvOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.occurences.declaration";
	...
	private de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvTokenScanner tokenScanner;
	...
	private de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ICvTextResource textResource;
	 ...
	 */
	public CvOccurrence(de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ICvTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.mopp.CvMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(de.tu_bs.cs.isf.mbse.cvclipse.resource.cv.ui.CvOccurrence.DECLARATION_ANNOTATION_ID);
	}
